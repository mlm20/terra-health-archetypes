    ...
    while (attempts <= MAX_RETRIES) {
        try {
            console.log(
                `Attempt ${
                    attempts + 1
                } to call OpenAI API for text generation using model: ${TEXT_MODEL}...`
            );
            ...
            if (!messageContent) {
                ...
                console.error(
                    "Invalid response structure from OpenAI API (Library): Missing content.",
                    completion
                );
                throw new Error(
                    "Invalid response structure from OpenAI API: Missing content."
                );
            }
            ...
            try {
                const parsedJson = JSON.parse(
                    messageContent
                ) as LLMArchetypeResponse;
                if (
                    !parsedJson.archetypeName ||
                    !parsedJson.archetypeDescription ||
                    !parsedJson.imagePrompt ||
                    !parsedJson.sliderValues
                ) {
                    throw new Error(
                        "Parsed JSON from OpenAI is missing required fields."
                    );
                }
                console.log(
                    "Successfully received and parsed archetype from OpenAI via library."
                );
                return parsedJson;
            } catch (parseError) {
                console.error(
                    "Failed to parse JSON response from OpenAI content (Library):",
                    parseError
                );
                console.error("LLM Response Content was:", messageContent);
                throw new Error(
                    `Failed to parse valid JSON from OpenAI response. Content: ${messageContent}`
                );
            }
        } catch (error) {
            console.error(
                `Error during OpenAI API call attempt ${
                    attempts + 1
                } (Library):`,
                error
            );
            if (attempts < MAX_RETRIES) {
                ...
                if (error instanceof OpenAI.APIError) {
                    ...
                    if (
                        ...
                        isRetryable = true;
                        console.warn(
                            `OpenAI API Error (${error.status}). Retrying after ${RETRY_DELAY_MS}ms...`
                        );
                    }
                } else if (
                    ...
                    isRetryable = true;
                    console.warn(
                        `Network error during OpenAI call. Retrying after ${RETRY_DELAY_MS}ms...`
                    );
                }
...
export async function generateArchetypeImage(
    ...
    let attempts = 0;
    while (attempts <= MAX_RETRIES) {
        // Using MAX_RETRIES from text generation for consistency
        try {
            console.log(
                `Attempt ${
                    attempts + 1
                } to call OpenAI Image API with model: ${IMAGE_MODEL}, prompt: "${imagePrompt.substring(
                    0,
                    100
                )}..."`
            );
            ...
            if (!response.data || response.data.length === 0) {
                console.error(
                    "Invalid response structure from OpenAI Image API: Missing data array or data array is empty.",
                    response
                );
                throw new Error(
                    "Invalid response structure from OpenAI Image API: Missing or empty data array."
                );
            }
            ...
            if (!imageBase64) {
                console.error(
                    "Invalid response structure from OpenAI Image API: Missing image b64_json data.",
                    response
                );
                throw new Error(
                    "Invalid response structure from OpenAI Image API: Missing image b64_json data."
            ...
            console.log(
                "Successfully received image data from OpenAI and constructed data URL."
            );
            return imageDataUrl;
        } catch (error) {
            console.error(
                `Error during OpenAI Image API call attempt ${attempts + 1}:`,
                error
            );
            if (attempts < MAX_RETRIES) {
                let isRetryable = false;
                if (error instanceof OpenAI.APIError) {
                    if (
                        ...
                        isRetryable = true;
                        console.warn(
                            `OpenAI Image API Error (${error.status}). Retrying after ${RETRY_DELAY_MS}ms...`
                        );
                    }
                } else if (
                    error instanceof Error &&
                    ...
                    isRetryable = true;
                    console.warn(
                        `Network error during OpenAI Image call. Retrying after ${RETRY_DELAY_MS}ms...`
                    );
                }
